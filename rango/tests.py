...
class IndexPageTests(TestCase):
    def test_index_contains_hello_message(self):
        ...
        self.assertIn(b'Rango says', bytes(response.content))
...
class AboutPageTests(TestCase):
    def test_about_contains_create_message(self):
        ...
        self.assertIn(b'This tutorial has been put together by', bytes(response.content))

    def test_about_contain_image(self):
        # Check if is there an image on the about page
        # Chapter 4
        response = self.client.get(reverse('about'))
        self.assertIn(b'img src="/static/', bytes(response.content))
...
class Chapter4ViewTests(TestCase):
    ...
    def test_does_index_contain_img(self):
        ...
        response = self.client.get(reverse('index'))
        self.assertIn(b'img', bytes(response.content))
...
class Chapter5ViewTests(TestCase):
    ...
    def test_view_has_title(self):
        ...
        self.assertIn(b'<title>', bytes(response.content))
        self.assertIn(b'</title>', bytes(response.content))
...
class Chapter6ViewTests(TestCase):
    ...
    def test_does_slug_field_work(self):
        ...
        cat.save()
        self.assertEqual(bytes(cat.slug), b'how-do-i-create-a-slug-in-django')
    """
